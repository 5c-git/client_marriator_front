...
import { RequestSearchDrawerInterface } from "~/shared/views/RequestSearchDrawer/RequestSearchDrawerInterface";
...
export class OrderMapper {
  ...
  static mapDataToSearch(data: PostCreateSearchFromOrderSuccess) {
    return {
      id: data.data.id,
      ...
      days: data.data.dateActivity.map((date) => {
  ...
  static mapLocationsToOptions(locations: GetPlaceForBidSuccess) {
  ...
  static mapSupervisorsToOptions(supervisors: GetSupervisorsForTaskSuccess) {
    return supervisors.data.map((item) => ({
      value: item.id.toString(),
      label: item.name,
      disabled: false,
    }));
