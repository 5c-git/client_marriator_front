...
import {
  postSearchSuccessSchema,
  PostSearchSuccess,
} from "./postUpdateSearchSuccess.schema";

import { UnxpectedError } from "~/shared/unexpectedError/unexpectedError";

export const postUpdateSearchKeys = ["postUpdateSearch"];

export type PostUpdateSearchPayload = {
  place: number;
  activity: number;
  amount: number;
  unitPrice: number;
...
export const postUpdateSearch = async (
  ...
  try {
    const url = new URL(import.meta.env.VITE_POST_UPDATE_SEARCH);
    ...
    const parsed = postSearchSuccessSchema.safeParse(response);

    if (parsed.success) {
      data = parsed.data;
    } else {
      console.log(parsed.error);
      throw new Response(`Данные запроса postUpdateSearch не валидны схеме`);
    }
